<template>
	<div class="cephService">
		...
		<h1 class="main-title">Start AI Analysis Service </h1>
		<div class="contentBox" v-if="order &&  order.product_obj">
			<div class="serviceRow">
				<div class="serviceRow__image">
					<img v-if="order.product_obj.images.length > 0"
				...
				<div class="serviceRow__title">
					{{ order.product_obj.parent_obj.title }} - {{ order.product_obj.title }}
					<div
					...
					>
						<router-link class="viewMore"
							:to="{
								...
								params: { lang: $i18n.locale, productId: String(order.product_obj.parent_obj.id)},
							}"
						>
						See More Details
				...
				<div class="serviceRow__progress">
					<b-progress :value="order.used_cases" :max="order.product_obj.subscription_size"></b-progress>
					<div class="d-flex justify-content-between align-items-center" v-if="order">
						<div v-if="!isDateBeforeToday(order.expire_date)" class="serviceRow__cases--remaining">
							<span v-html="order.product_obj.subscription_size - order.used_cases"></span>
							cases remaining
							<svg-icon
								v-b-tooltip.hover
		...
		<div class="pageHead" v-if="order">
			<div class="textWrapper">
				...
				<p>Select or add a patient's record to get started!!</p>
			</div>
		</div>
		<div class="d-flex patientRecord">
			<div class="patientRecord__left">
				<div class="form-group">
					<b-dropdown :text="selected_patient_display" v-model="selected_patient">
						<b-dropdown-text>
							<div class="form-group no-label search-input">
								<input
									type="text"
									class="form-control"
									placeholder="Search by Patient Name"
									v-model="searchByPatient"
								/>
							</div>
							Or
						...
						<b-dropdown-item href="#" v-for="(patient, index) in patientList" :key="index">
							<div @click="updateSelectedPatient(patient)">
								<span>{{ patient.first_name }} {{ patient.last_name }}</span>{{ patient.age }} years
							</div>
						</b-dropdown-item>
					</b-dropdown>
			...
			<div class="patientRecord__right" v-if="selected_patient && selected_record">
				<div class="form-group">
					<b-dropdown :text="selected_record_display" v-model="selected_record" >
						<b-dropdown-item href="#" v-for="(record, index) in patientAiServices" :key="index" @click.prevent="updateSelectedRecord(index)">
			...
			<div class="patientRecord__button">
				<button v-if="order ? !isDateBeforeToday(order.expire_date) : false" href="#" class="btn btn-primary" @click="createNewRecord" :disabled="!selected_patient || (order.product_obj.subscription_size - order.used_cases) < 1">
					<span class="icon">
						<svg-icon
							icon-id="plus-icon"
    	...
    	/>
		<div class="contentBox analysis" style="padding: 0px">
			<iframe
				v-if="selected_record"
				:src="`https://voxel3di.web.app/service_id=${selected_record.id}`"
				name="new_ifram"
				id="frame"
				width="100%"
				class="frame"
			...
			/>
			<div v-if="patientAiServices.length == 0" class="noService">
				<img
...
import CephRecords from "../components/service/CephRecords.vue";

// [*] Import Breadcrumbs
import { startCephServiceAnalysisBreadCrumbs } from "@/common/constant/breadCrumbs"
...
export default {
	...
	data() {
		return {
			breadCrumbsItems: [...startCephServiceAnalysisBreadCrumbs],
			myIframe: null,
			
			order: null,
			service_patient: null,
	...
	computed: {
		...mapGetters(['orderLine', 'patients', 'patientAiServices', 'serviceDetail']),
	},
	mounted() {
		this.type == 'order'
	...
	watch: {
		...
		serviceDetail() {
			...
			this.service_patient = this.serviceDetail.patient_obj;
		},
		patients(){
			if(this.patients.length == 0) return;

			// Show only last 5 add patient on patients list...
			this.patientList = []
			this.patients.forEach((value, index, array) => {
				if(index > this.patients.length - 6) this.patientList.push(value)
			})
		},
		service_patient() {
			if(this.service_patient != null) this.updateSelectedPatient(this.service_patient)
		},
		patientAiServices() {
		...
		searchByPatient() {
			this.patientList = []
			if(this.searchByPatient.length > 0) {
				ServiceHelper.searchPatient({name: this.searchByPatient}).then((data) => {
	...
	methods: {
		...
		updateSelectedPatient(data){
			this.selected_patient = data;
			this.selected_patient_display = this.selected_patient.first_name + " " + this.selected_patient.last_name;
			this.getPatientAiServices(data.id)
		},
		updateSelectedRecord(index){
			this.selected_record = this.patientAiServices[index];
			this.selected_record_display = moment(String(this.selected_record.created)).format('MM/DD/YYYY hh:mm');
		},
